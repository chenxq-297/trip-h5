...
const useDetail = defineStore("detail", {
  ...
  getters: {
    swipeData: (state) =>
      state.detailInfos.mainPart?.topModule.housePicture.housePics,
    topInfos: (state) => state.detailInfos.mainPart?.topModule,
    houseFacility: (state) =>
      state.detailInfos.mainPart?.dynamicModule.facilityModule.houseFacility,
    landlord: (state) =>
      state.detailInfos.mainPart?.dynamicModule.landlordModule,
    comment: (state) => state.detailInfos.mainPart?.dynamicModule.commentModule,
    orderRules: (state) =>
      state.detailInfos.mainPart?.dynamicModule.rulesModule.orderRules,
    position: (state) =>
      state.detailInfos.mainPart?.dynamicModule.positionModule,
    introduction: (state) => state.detailInfos.mainPart?.introductionModule,
    currentHouse: (state) => state.detailInfos.currentHouse,
  },
  actions: {
